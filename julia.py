...
class Julia_set:
    ...
    def update_chewing(self):
        ...
        fine = 0.01
        coarse = 0.1
        # Shift key provides a coarser, faster adjustment.
        step = coarse if pyxel.btn(pyxel.KEY_SHIFT) else fine

        # Allow keyboard input for fine-tuning the C constant.
        if self.input_delay == 0:
            ...
            if pyxel.btn(pyxel.KEY_DOWN):
                self.c_imag += step
            self.input_delay = 3  # Set delay to prevent rapid key repeats.

        # Reduce input delay counter.
        if self.input_delay > 0:
            self.input_delay -= 1
